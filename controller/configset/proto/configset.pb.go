...
func (m *ConfigSet) CloneVT() *ConfigSet {
	...
	if rhs := m.Configs; rhs != nil {
		r.Configs = make(map[string]*ControllerConfig, len(rhs))
		for k, v := range rhs {
			r.Configs[k] = v.CloneVT()
		}
	}
...
func (x *ConfigSet) MarshalProtoText() string {
	...
	if len(x.Configs) > 0 {
		...
		for _, k := range slices.Sorted(maps.Keys(x.Configs)) {
			v := x.Configs[k]
			sb.WriteString(" ")
			sb.WriteString(strconv.Quote(k))
			sb.WriteString(": ")
			if v == nil {
				sb.WriteString((&ControllerConfig{}).MarshalProtoText())
			} else {
