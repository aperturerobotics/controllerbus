...
func CodegenPluginWrapperFromAnalysis(
	...
	// ControllerBusHotPlugin const
	allDecls = append(allDecls, &gast.GenDecl{
		Doc: &gast.CommentGroup{
			List: []*gast.Comment{{
				Text: "// " + hot_loader.HotPluginGlobalVar + " is the variable read by the plugin loader.",
			}},
		},
		Tok: token.VAR,
		Specs: []gast.Spec{
			&gast.ValueSpec{
				Names: []*gast.Ident{gast.NewIdent(hot_loader.HotPluginGlobalVar)},
				Type: &gast.SelectorExpr{
					X:   gast.NewIdent("hot_plugin"),
